module EBN #(
    ...
    output logic [7:0] o_x_0,
    output logic [7:0] o_x_1,
    output logic [7:0] o_x_est_0,
    output logic [7:0] o_x_est_1,
    output logic [7:0] pot_out,
    output logic [7:0] ws_out,
    output logic       done
);
    ...
    logic       load_complete;
    logic [7:0] i_wf             [N];
    logic [7:0] pot_thresh       [N];
    logic [7:0] randn            [N];
    logic [7:0] i_cmd            [Dims];
    logic [7:0] i_dec            [N* Dims];

    // Delayed Signals for Synaptic_core
    logic [7:0] old_spike_f_data [N];
    logic [7:0] old_err          [Dims];
    ...
    logic       done_spike_f;
    logic [7:0] spife_f_add;
    logic [7:0] o_spike_f        [N];
    
    // Signals from Desired_Dynamic
    logic       done_dyn; 
    logic [7:0] o_err       [Dims];
    logic [7:0] o_x         [Dims];
    logic [7:0] o_x_est     [Dims];

    // Signals for Spike_filter sync
    logic [7:0] spike_f_data     [N]; 
