module memory_manager #(
    ...
    output logic [7:0] i_wf       [N],
    output logic [7:0] i_dec      [Dims * N],
    output logic [7:0] pot_thresh [N],
    output logic [7:0] randn      [N],
    output logic [7:0] i_cmd      [Dims]
);

    // BRAM interfaces for i_wf, i_dec, pot_thresh, randn, and i_cmd (preloaded from COE files)
    logic [7:0]  pot_thresh_data;
    logic [7:0]  i_wf_data;
    logic [7:0]  i_dec_data;
    logic [7:0]  randn_data;
    logic [7:0]  cmd_data;
    // BRAM read delay
    ...
    always_ff @(posedge clk) begin
        ...
        else if (init_signal && !load_complete_thr) begin
            // BRAM read delay
            if (read_delay_counter_init < 2) begin
